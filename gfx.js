...
dkc2ldd.gfx = (function(app=dkc2ldd){
	...
	o.fast.decode_4bppTileset_OLD = function(data, hFlip=0,vFlip=0){
	...
	o.safe.decode_4bppTileset_OLD = function(data, hFlip=0,vFlip=0){
	...
	o.fast.draw_decodedTileset_OLD = function(data, palette, hFlip=0,vFlip=0, xtmax, ctx,x=0,y=0){
	...
	o.safe.draw_decodedTileset_OLD = function(data, palette, hFlip=0,vFlip=0, xtmax, ctx,x=0,y=0){
	...
	// o.fast.format_4bppTileset has been moved in Tileset.js
	...
	// o.fast.draw_formatedTileset has been moved in Tileset.js
	...
	o.fast.animatedTiles_to_vramBackRef = function(animations, vramRefs, iFrame, backRef){
		...
		for(let iAnim=0; iAnim<len; iAnim++){
			...
			//	let srcOfst = iFrame * frameBytes;
				let frame = iFrame % anim.frameCount;
				let srcOfst = anim.srcOffsets[frame];
	...
	o.fast.vramTileset_to_animatedTiles = function(animations, vramRefs, vramTileset, iFrame){
		...
		for(let iAnim=0; iAnim<len; iAnim++){
			...
			if(anim){
				...
				let frame = iFrame % anim.frameCount;
				let dstOfst = anim.srcOffsets[frame];
